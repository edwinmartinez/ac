...
function phpbb_login( $phpbb_user_id ) {
...
if ($useDatabase == true) {
	...
	//$userQuery = mysql($cfgDbDatabase, "SELECT * FROM users WHERE username = '$login'")
	...
	if (mysql_num_rows($userQuery) != 0) {
		...
		if (strtolower($login) != strtolower($userArray[$cfgDbLoginfield])) {
			// Case in-sensitive user not present in database
...
//		include($cfgProgDir . "logout.php");
	...
	if ( isset($userArray["$cfgDbUserIDfield"]) && !empty($cfgDbUserIDfield) ) {
		...
		$_SESSION['user_id'] = intval($ID);				        $_SESSION['username'] = $userArray[$cfgDbLoginfield];
		
		// Then login the user to the PHPBB
		phpbb_login(intval($ID));
		
		//record login date on db
		$sql = "UPDATE ".SITE_USERS_TABLE." SET ".USER_LAST_LOGIN_FIELD."= now() where ".USER_ID_FIELD."=".$_SESSION['user_id'];
		mysql_connect($cfgServerHost . ":" . $cfgServerPort, $cfgServerUser, $cfgServerPassword)
		or die($strNoConnection);
		$updateLoginDateQuery = mysql($cfgDbDatabase, $sql)
		or die($strNoDatabase);
