 ...
 * 
 * <code><?php
 * include('useraccess.class.php');
 * $user = new userAccess();
 * ? ></code>
 * 
 * For support issues please refer to the webdigity http://www.biggreenbeats.com/wordpress/wp-content/uploads/2011/06/Sexy-and-I-Know-It.mp3forums :
...
class userAccess{
  ...
  var $dbTable  = 'users';
  /**
   * The session variable ($_SESSION[SESSION_VARIABLE]) which will hold the data while the user is logged on
   * var string
   */
  //var $sessionVariable = 'acLoggedUser'; //defined in config
  /**
  ...
  function __construct($dbConn = '', $settings = '')
  {
	    ...
	    if ( !empty($_SESSION[SESSION_VARIABLE]) )
	    {
		    $this->loadUser( $_SESSION[SESSION_VARIABLE] );
	    }
  ...
  */
  function login($uname, $password, $remember = false, $loadUser = true)
  {
		...
		if ( $loadUser )
		{
			...
			$_SESSION[SESSION_VARIABLE] = $this->userID;
			if ( $remember ){
  ...
  */
  function loadUser($userID)
  {
    ...
    //record login date on db
		//$sql = "UPDATE `{$this->dbTable}` SET ".USER_LAST_LOGIN_FIELD."= now() WHERE `{$this->tbFields['userID']}` = '".$this->escape($userID)."' LIMIT 1";
    $_SESSION[SESSION_VARIABLE] = $this->userID;
    return true;
  }
  ...
  */  
  ...
  /**
   * Determine if we have to provide the login form 
  ...
  function checkIn(){
	...
	if (!$this->is_loaded()) {
		...
		if ( isset($_POST['username']) && isset($_POST['password'])){
		  if ( !$this->login($_POST['username'],$_POST['password'],$_POST['remember'] )){//Mention that we don't have to use addslashes as the class do the job
		...
		//echo "logged in";
		$redirect_to = ($_GET['redirect']) ? $_GET['redirect'] : LOGIN_REDIRECT_DEFAULT;
		if (strstr(urldecode($redirect_to), "\n") || strstr(urldecode($redirect_to), "\r"))
		{
  ...
  function printLoginForm(){
	...
	$this->smarty->display('block_login.tpl');	
  }
}
?>
