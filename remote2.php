...
function change_info(){
	...
	if ($info_to_change == 'email' && check_email($email) && isset($HTTP_SESSION_VARS['user_id']) ) {
		$sql = "UPDATE phpbb_users set user_email=".GetSQLValueString($email,"text")." WHERE user_id =".$HTTP_SESSION_VARS['user_id'];
		mysql_connect($dbhost, $dbuser, $dbpasswd) or
			die("Could not connect: " . mysql_error());
		mysql_select_db($dbname);
		if ( !($result = mysql_query($sql))){
...
function delete_fav(){
	...
	//let's check if the user has allready requested the addition
	$sql = "DELETE from ".FAVORITE_PEOPLE_TABLE." WHERE user_uid = ".$user_id." AND fav_uid = ".$fav_id;
	if($result = mysql_query($sql)){
		...
		//-------------------------------------------------------------------------------
			...
			if($comment != ""){
				...
				if($userinfo['comments_on_photo_need_approval'] == 0)
					$sql .= " approved,";
				$sql .= " date) ";
				$sql .= "VALUES(".$photo_id.", ".$commenter_uid.", ".GetSQLValueString($comment,"text").", ";
				if($userinfo['comments_on_photo_need_approval'] == 0)
					$sql .= " 1,";
				$sql .=	" NOW())";
				if ( !($result = mysql_query($sql))){
