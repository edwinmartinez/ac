...
//		var status = objDomTree.getElements("status")[0].getText();
		...
		theTitle= '<h2>'+title+'</h2>';

		picControls='<div class="myPicturesControls">';
		picControls+='<div style="float:left"><img src="/images/icon_cross.png"></div>';
		...
		imgobj.innerHTML=theTitle+returnimgcode(dynimages[imgindex])+picCaption;
...
//		//tags area-------------------------------------------------------------------------------
		...
		for(var i=0;i<tagsNodes.length;i++){
			...
			//tagsobj.innerHTML+= ' <a href="#" onclick="remove_tag('+tagid+','+theimg[0]+'); return false;">(x)</a>
			tagsobj.innerHTML+='<br>';
		}
		picid = theimg[0];
