...
function display_dbimages(){
	...
	if($profile_pic_rows > 0){
		...
		while($profile_pic_row = mysql_fetch_assoc($profile_pic_result)){
			...
			list($id,$basename,$ext) = split('[.-]',$profile_pic);
			//check if medium size exists
			$medfile  = $id.'.'.$basename.'_m.'.$ext;
			$largestfile = $id.'.'.$basename.'_l.'.$ext;
			
			if (file_exists(MEMBER_IMG_DIR_PATH.'/'.$uid.'/'.$medfile)) {
				$profile_pic = $medfile;
			}else{
				$profile_pic = $largestfile;
			}
			//check that we printed the first item cause javascript is picky about that last comma
			if($first_item_printed == 1){	echo ', '."\n"; }
			echo '["' . $profile_pic .'","'.$profile_pic_row['photo_title'].'"]';
...
function returnimages() {
	$dirname="../membergallery/"+$uid;
   $pattern="\.(jpg|jpeg|png|gif|bmp)$";
   $files = array();
   $curimage=0;
   $count = returndbimages();
...
var JaS = {
	...
	imagePath : "<?php echo MEMBER_IMG_DIR_URL.'/'.$uid; ?>/",
	images : //JJ.images,
	[
	<?php display_dbimages(); ?>

//["60.b6f382e3b0ca25984b724e19b9e5aea6_m.jpg", "Woode Wood playing guitar", "Woode Wood, River park"],
//	["60.ee94ef72f2e6af7718023e3de09e0959_m.jpg", "Bat bridge in Austin", "Bridge"] // Separate multiple tags by a comma
	...
	createThumbnails : function (){
		...
		for(var i=0; i<this.currentImages.length; i++){
			...
			oThumbnail.setAttribute("src", (this.imagePath + oCurrentImage[0].replace(/_l|_m/,'_sq')));
