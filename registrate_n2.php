...
<meta name="description" lang="es" content="Servicio de citas en linea en donde podras conocer, de una manera facil y simple, a otros solteros en Latinoamerica y el resto del mundo en busca de amor y amistad." />
...
function validate() {
	...
	if ( ! f.first_name.value.isAT_SIGN() ) {
		alert ("Escribe de nuevo tu Nombre utilizando unicamente letras");
		f.first_name.focus()
		changeClass('firstNameArea','error');
		return
	}
	...
	if ( ! f.last_name.value.isAT_SIGN() ) {
		alert ("Escribe de nuevo tu apellido utilizando unicamente letras");
		f.last_name.focus()
		changeClass('lastNameArea','error');
		return
	}
	// Comprobamos que en el nombre no nos pongan numeros
	if( f.last_name.value.indexOf("0") != -1 ||
		...
		f.last_name.value.indexOf("9") != -1 ) {
			alert ("El apellido no puede contener caracteres numericos");
			f.last_name.focus()
			changeClass('lastNameArea','error');
			return
	}
	...
	if( !fechaCorrecta(fnac_dia + "/" + fnac_mes + "/" + fnac_ano) ) {
		alert("La 'Fecha nacimiento' no es una fecha valida.");
		f.birth_day.focus();
	...
	// Comprobamos que el usuario tenga mas de 18 a&ntilde;os
	// format 2005/12/21
	...
	if ( ! f.username.value.isAT_SIGN() ) {
		alert ("Escribe de nuevo tu Apodo utilizando unicamente letras, cifras y guiones bajos");
		f.username.focus()
		changeClass('userNameArea','error');
		return
	}
	...
	if ( f.password.value == '' ) {
		alert ("Contrase&ntilde;a es obligatoria");
		f.password.focus()
	...
	if ( f.confirm_password.value == '' ) {
		alert ("Escribe de nuevo tu contrase&ntilde;a");
	...
	if ( f.password.value != f.confirm_password.value ) {
		alert ("Los campos de las contrase&ntilde;as no coinciden");
...
// Chequea si la fecha es valida segun el formato dd/mm/yyyy
...
// Funcion que comprueba que no haya @
...
// Funcion que comprueba si el email es correcto
